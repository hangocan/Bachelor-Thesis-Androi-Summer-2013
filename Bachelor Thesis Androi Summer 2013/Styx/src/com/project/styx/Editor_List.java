 ...
 */
public class Editor_List extends ListActivity {
	/* Global Variable Declarations */
	private static final int intDefaultSortOption = -1;
	static int intSortOption = Editor_List.intDefaultSortOption;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Editor_List.onCreate", "");
			errExcpError = null;
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj
						.fetchEditor(Editor_List.intSortOption);
				...
				int[] to = new int[] { R.id.txteditorfullname, R.id.txteditorhomepage };
				av = (ListView) findViewById(android.R.id.list);
				// Now create a simple cursor adapter and set it to display
				listRegister = new SimpleCursorAdapter(this,
						R.layout.editorentryrow, mEntryCursor, from, to);
				listRegister.setFilterQueryProvider(new FilterQueryProvider() {
					public Cursor runQuery(CharSequence constraint) {
						...
						return dbAppDbObj.fetchEditorByName(partialItemName);
					}
				});
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error, "Editor_List.fillData", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Editor_List.this,
							AddEditEditor.class);
					...
					prefsIntent.putExtra("PREFERENCE_FILE",
							R.xml.editor_view_prefs);
					prefsIntent.putExtra("PREFERENCE_KEY",
							getString(R.string.EditorSortOpt));
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_EDITOR_DB_TABLE,
												MyAppDbAdapter.KEY_EDITORFULLNAME
														+ ","
														+ MyAppDbAdapter.KEY_EDITORHOMEPAGE
														+ ","
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_EDITOR_DB_TABLE);
								Editor_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Editor_List.this,
					...
					// end try
						...
						errExcpError.addToLogFile(error,
								"Editor_List.onMenuItemSelected",
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Editor_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditEditor.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_EDITORFULLNAME,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_EDITORFULLNAME)));
				i.putExtra(
						MyAppDbAdapter.KEY_EDITORHOMEPAGE,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_EDITORHOMEPAGE)));
				i.putExtra(
						MyAppDbAdapter.KEY_EDITOREMAIL,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_EDITOREMAIL)));
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Editor_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbEditor == null)
					switch (requestCode) {
					case ACTIVITY_CREATE:
						if (resultCode == RESULT_OK && intent != null) {
							String txtEditorfullname = extras
									.getString(MyAppDbAdapter.KEY_EDITORFULLNAME);
							String txtEditorhomepage = extras
									.getString(MyAppDbAdapter.KEY_EDITORHOMEPAGE);
							String txtEditoremail = extras
							...
							if (dbOpenResult) {
								boolean blIsSuccessful = this.dbAppDbObj
										.createEditorEntry(txtEditorfullname,
												txtEditorhomepage,
												txtEditoremail);
								boolean dbCloseResult = this.dbAppDbObj
										.closeDbAdapter();
					...
					case ACTIVITY_EDIT:
						if (resultCode == RESULT_OK && intent != null) {
							...
							if (rowId != null) {
								String strEditor = extras
										.getString(MyAppDbAdapter.KEY_EDITORFULLNAME);
								String strHomepage = extras
										.getString(MyAppDbAdapter.KEY_EDITORHOMEPAGE);
								String strEmail = extras
										.getString(MyAppDbAdapter.KEY_EDITOREMAIL);
								String strOrigEditor = extras
										.getString("OrigEditor");
								...
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.updateEditorEntry(rowId,
													strEditor, strHomepage,
													strEmail, strOrigEditor,
													strOrigHomepage,
													strOrigEmail);
									boolean dbCloseResult = this.dbAppDbObj
											.closeDbAdapter();
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError
					.addToLogFile(error, "Editor_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Editor_List.objDisplayAlertClass != null) {
				Editor_List.objDisplayAlertClass.cleanUpClassVars();
				Editor_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error, "Editor_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected static void getSortOption(Context ctxContext) {
		try {
			GetSortOptions objGetSortOptions = new GetSortOptions(ctxContext,
					"Editor_List", "EditorSortOpt");
			Editor_List.intSortOption = objGetSortOptions.getIntSortOption("");
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					ctxContext);
			errExcpError
					.addToLogFile(error, "Editor_List.getSortOption",
							"An Error Occured, Sort Option set to default, sorted by rowID");
			errExcpError = null;
			Editor_List.intSortOption = Editor_List.intDefaultSortOption;
		}// end try/catch (Exception error)
	}// end getSortOption
	...
	protected void onResume() {
		...
		try {
			if (this.blSaveNew == true) {
				...
				// set database query class object
			this.dbAppDbObj = new MyAppDbSQL(this);
			Editor_List.getSortOption(Editor_List.this);
			this.fillData();
			this.registerForContextMenu(this.getListView());
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error, "Editor_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	...
	private void displayConfirmRequest(int id, final MenuItem item) {
		try {
			...
			case DELETE_CONFIRMATION_MESSAGE:
				new AlertDialog.Builder(Editor_List.this)
								...
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
										...
										AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
												.getMenuInfo();
										try {
											boolean dbOpenResult = Editor_List.this.dbAppDbObj
													.openDbAdapter();
											if (dbOpenResult) {
												boolean blIsSuccessful = Editor_List.this.dbAppDbObj
														.deleteEditorEntry(info.id);
												boolean dbCloseResult = Editor_List.this.dbAppDbObj
														.closeDbAdapter();
												if (!dbCloseResult)
													throw new Exception(
															"The database was not successfully closed.");
											...
											errExcpError
													.addToLogFile(
															error,
															"Editor_List.displayConfirmRequest.deleteEditorEntry",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
									...
									public void onClick(DialogInterface dialog,
										...
										try {
											...
											errExcpError
													.addToLogFile(
															error,
															"Editor_List.displayConfirmRequest.deleteEditorEntries",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error,
					"Editor_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error, "Editor_List.onSaveInstanceState",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Editor_List.this);
			errExcpError.addToLogFile(error,
					"Editor_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
