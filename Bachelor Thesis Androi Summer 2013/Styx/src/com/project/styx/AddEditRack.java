...
public class AddEditRack extends Activity {
	
	/** Define variables to use in the activity **/
	private EditText txtRacklabel;
	
	private EditText txtRackinfo1;
	
	private String strOrigRack = "";
	
	private String strOrigRackinfo1 = "";
	
	public Integer OrigSpinnerrack = 0;
	
	private MyAppDbSQL dbAppDbObj;
	
	private Long mRowId;
	
	private Spinner spinnerrack;
	
	private Cursor spinnerrackcursor;
	 ...
	 * field */
	private OnFocusChangeListener txtRackOnFocusChangeListener = new OnFocusChangeListener() {
	...
	private OnFocusChangeListener txtRackinfo1OnFocusChangeListener = new OnFocusChangeListener() {
	...
	protected void myAppCleanup() {
		...
		if (this.spinnerrackcursor != null) {
			this.spinnerrackcursor.close();
			this.spinnerrackcursor = null;
		}
	...
	private void myCancelMethod() {
		...
		bundle.putBoolean("IS_SAVE_NEW", AddEditRack.this.blSaveNew);
	...
	private void mySaveDataMethod() {
		...
		if (this.txtRacklabel == null) {
			this.txtRacklabel = (EditText) findViewById(R.id.txtRacklabel);
		}
		try {
			/** Define the condition command for must filled field ***************************************/
			strRack = this.txtRacklabel.getText().toString();
			if (strRack.contentEquals("")) {
				objDisplayAlertClass = new MyDisplayAlertClass(AddEditRack.this,
						new CustAlrtMsgOptnListener(MessageCodes.ALERT_TYPE_MSG),
						"Missing Required Rack label ", "Please enter the rack ");
				boolSaveOk = false;
				txtRacklabel.requestFocus();
			}
			/******************************************************************************************/
			strRackinfo1 = this.txtRackinfo1.getText().toString();
			// ////////////////////////////////////////////////////////////
			spinSpinnerrack = (int) this.spinnerrack.getSelectedItemId();
			// ////////////////////////////////////////////////////////////
			/** Save file command **/
			if (boolSaveOk == true) {
				APPGlobalVars.SCR_PAUSE_CTL = "SAVE";
				Bundle bundle = new Bundle();
				bundle.putString(MyAppDbAdapter.KEY_RACK_LABEL, this.txtRacklabel.getText().toString());
				bundle.putString(MyAppDbAdapter.KEY_RACK_INFO_1, strRackinfo1);
				bundle.putInt(MyAppDbAdapter.KEY_RACK_ROOMID, spinSpinnerrack);
				bundle.putBoolean("SHOW_FILTER", true);
				if (mRowId != null) {
					bundle.putLong(MyAppDbAdapter.KEY_ROWID, mRowId);
				}
				if (((!strOrigRack.equals(strRack)) && (!strOrigRack.equals("")))) {
					bundle.putString("OrigRack", strOrigRack);
				} else {
					bundle.putString("OrigRack", "");
				}// end if
				if (((!strOrigRackinfo1.equals(strRackinfo1)) && (!strOrigRackinfo1.equals("")))) {
					bundle.putString("OrigRackinfo1", strOrigRackinfo1);
				} else {
					bundle.putString("OrigRackinfo1", "");
				}// end if
					// ///////////////////////////////////////////////////////
				if (((!OrigSpinnerrack.equals(spinSpinnerrack)) && (!OrigSpinnerrack.equals(0)))) {
					bundle.putInt("OrigSpinnerrack", OrigSpinnerrack);
				} else {
					bundle.putInt("OrigSpinnerrack", 0);
				}// end if
					// ////////////////////////////////////////////////////////////
				bundle.putBoolean("IS_SAVE_NEW", AddEditRack.this.blSaveNew);
				Intent mIntent = new Intent();
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.mySaveDataMethod", "");
			errExcpError = null;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		try {
			AddEditRack.this.blSaveNew = false;
			txtRacklabel = (EditText) findViewById(R.id.txtRacklabel);
			txtRackinfo1 = (EditText) findViewById(R.id.txtRackinfo1);
			// ///////////////////////////////////////////////
			this.spinnerrack = (Spinner) findViewById(R.id.spinnerrack);
			// ///////////////////////////////////////////////
			if (intent != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.add_edit_rack);
		fillspinnerrack();
		try {
			try {
				...
				Bundle extras = getIntent().getExtras();
				if (extras != null) {
					String strRack = extras.getString(MyAppDbAdapter.KEY_RACK_LABEL);
					String strRackinfo1 = extras.getString(MyAppDbAdapter.KEY_RACK_INFO_1);
					int intSpinnerrack = Integer.parseInt(extras
							.getString(MyAppDbAdapter.KEY_RACK_ROOMID));
					...
					if (strRack != null && !strRack.equals("") && !strRack.equals(" ")) {
						txtRacklabel.setText(strRack);
						strOrigRack = strRack;
					}
					if (strRackinfo1 != null && !strRackinfo1.equals("") && !strRackinfo1.equals(" ")) {
						txtRackinfo1.setText(strRackinfo1);
						strOrigRackinfo1 = strRackinfo1;
					}
					Integer value = Integer.valueOf(intSpinnerrack);
					if (value != null && intSpinnerrack != 0) {
						OrigSpinnerrack = intSpinnerrack;
					}
				}// end if
				/*** This code set event text focus change listeners */
				txtRacklabel.setOnFocusChangeListener(txtRackOnFocusChangeListener);
				txtRackinfo1.setOnFocusChangeListener(txtRackinfo1OnFocusChangeListener);
			}// end try statements
			catch (Exception error) {
				MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
				errExcpError.addToLogFile(error, "AddEditRack.onCreate", "");
				errExcpError = null;
			}// end try/catch (Exception error)
		}
		catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "AddEditRack.onCreate", "opening database");
			errExcpError = null;
		}
	}// end onCreate
	
	private void fillspinnerrack() {
		try {
			boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
			if (dbOpenResult) {
				spinnerrackcursor = dbAppDbObj.fetchRoom();
				this.startManagingCursor(this.spinnerrackcursor);
				spinnerrackcursor.moveToFirst();
				spinnerrack = (Spinner) findViewById(R.id.spinnerrack);
				String[] from = new String[] { MyAppDbAdapter.KEY_ROOM_NUMBER, MyAppDbAdapter.KEY_ROWID };
				int[] to = new int[] { android.R.id.text1 };
				spinneradapter = new SimpleCursorAdapter(AddEditRack.this,
						android.R.layout.simple_spinner_item, this.spinnerrackcursor, from, to);
				spinnerrack.setAdapter(this.spinneradapter);
				Bundle extras = getIntent().getExtras();
				int intSpinnerrack = Integer.parseInt(extras.getString(MyAppDbAdapter.KEY_RACK_ROOMID));
				for (int i = 0; i < spinnerrack.getAdapter().getCount(); i++) {
					int id = (int) spinnerrack.getItemIdAtPosition(i);
					if (id == intSpinnerrack) {
						spinnerrack.setSelection(i);
					}
				}
			}
		}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.fillspinnerrack ", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}
		...
		try {
			switch (keyCode) {
				case KeyEvent.KEYCODE_ENTER:
					if (txtRacklabel.hasFocus()) {
						txtRackinfo1.requestFocus();
						return true;
					}
				default:
					return false;
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.onKeyUp", "");
			errExcpError = null;
			return false;
		}// end try/catch (Exception error)
	}// end onKeyUp
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		if (itemId == R.id.save_menu) {
			...
			Intent intentQuitActivity = new Intent(AddEditRack.this, Rack_List.class);
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			if (savedInstanceState != null) {
				String strRack = savedInstanceState.getString(MyAppDbAdapter.KEY_RACK_LABEL);
				String strRackinfo1 = savedInstanceState.getString(MyAppDbAdapter.KEY_RACK_INFO_1);
				int intSpinnerrack = savedInstanceState.getInt(MyAppDbAdapter.KEY_RACK_ROOMID);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.onRestoreInstanceState", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			savedInstanceState.putString(MyAppDbAdapter.KEY_RACK_LABEL, txtRacklabel.getText()
					.toString());
			savedInstanceState.putString(MyAppDbAdapter.KEY_RACK_INFO_1, txtRackinfo1.getText()
					.toString());
			savedInstanceState.putInt(	MyAppDbAdapter.KEY_RACK_ROOMID,
												(int) spinnerrack.getSelectedItemId());
			if (mRowId != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(AddEditRack.this);
			errExcpError.addToLogFile(error, "AddEditRack.onSaveInstanceState", "");
			errExcpError = null;
		}// end try/catch (Exception error)
