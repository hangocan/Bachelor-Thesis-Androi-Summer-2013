 ...
 */
/** Class for displaying a list of data, activity screen and options. */
public class Room_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Room_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				this.mEntryCursor = this.dbAppDbObj.fetchRoom();
				this.startManagingCursor(this.mEntryCursor);
				String[] from = new String[] { MyAppDbAdapter.KEY_ROOM_NUMBER };
				int[] to = new int[] { R.id.roomnumber };
				listRegister = new SimpleCursorAdapter(Room_List.this, R.layout.roomentryrow,
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.fillData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end fillData
		...
		try {
			switch (item.getItemId()) {
				case R.id.new_entry_menu_option:
					Intent intentInsrt = new Intent(Room_List.this, AddEditRoom.class);
				...
				case R.id.export_list:
					...
					try {
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(MyAppDbAdapter.MY_ROOM_DB_TABLE,
																MyAppDbAdapter.KEY_ROWID + " ASC");
								this.startManagingCursor(this.mEntryCursor);
								boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
								if (!dbCloseResult)
									throw new Exception("The database was not successfully closed.");
								DBTableExport objDBTableExport = new DBTableExport(Room_List.this);
								objDBTableExport.exportAsCSVFile(cursorTableQuery,
																			MyAppDbAdapter.MY_ROOM_DB_TABLE);
								Room_List.objDisplayAlertClass = new MyDisplayAlertClass(Room_List.this,
						...
						MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
						errExcpError.addToLogFile(	error, "Room_List.onMenuItemSelected",
															"attempting to export the "
																	+ MyAppDbAdapter.MY_ROOM_DB_TABLE + " table.");
						errExcpError = null;
						return false;
					}// end try/catch
				case R.id.quit:
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditRoom.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(MyAppDbAdapter.KEY_ROOM_NUMBER, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_ROOM_NUMBER)));
				i.putExtra(MyAppDbAdapter.KEY_ROOM_INFO_1, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_ROOM_INFO_1)));
				i.putExtra(MyAppDbAdapter.KEY_ROOM_STORAGEID, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_ROOM_STORAGEID)));
				// bgh 08/26/2010 1.03
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Room_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbRoom == null)
					switch (requestCode) {
						case ACTIVITY_CREATE:
							if (resultCode == RESULT_OK && intent != null) {
								String txtRoomnumber = extras.getString(MyAppDbAdapter.KEY_ROOM_NUMBER);
								String txtRoominfo1 = extras.getString(MyAppDbAdapter.KEY_ROOM_INFO_1);
								int spinSpinnerroom = extras.getInt(MyAppDbAdapter.KEY_ROOM_STORAGEID);
								
							//	int OrigSpinnerroom = extras.getInt("OrigSpinnerroom");
								// save the data to the database table
								boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.createRoomEntry(txtRoomnumber, txtRoominfo1, spinSpinnerroom);
									boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
									if (!dbCloseResult)
										throw new Exception("The database was not successfully closed.");
									if (blIsSuccessful == false) {
										Room_List.objDisplayAlertClass = new MyDisplayAlertClass(
												Room_List.this, new CustAlrtMsgOptnListener(
						...
						case ACTIVITY_EDIT:
							if (resultCode == RESULT_OK && intent != null) {
								...
								if (rowId != null) {
									String strRoomnumber = extras.getString(MyAppDbAdapter.KEY_ROOM_NUMBER);
									String strRoominfo1 = extras.getString(MyAppDbAdapter.KEY_ROOM_INFO_1);
									String strOrigRoomnumber = extras.getString("OrigRoomnumber");
									String strOrigRoominfo1 = extras.getString("OrigRoominfo1");
									int spinSpinnerroom = extras.getInt(MyAppDbAdapter.KEY_ROOM_STORAGEID);
									int OrigSpinnerroom = extras.getInt("OrigSpinnerroom");
									// save the data to the database table
									boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
									if (dbOpenResult) {
										boolean blIsSuccessful = this.dbAppDbObj
												.updateRoomEntry(rowId, strRoomnumber, strRoominfo1,
																		spinSpinnerroom, strOrigRoomnumber,
																		strOrigRoominfo1, OrigSpinnerroom);
										boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
										if (!dbCloseResult)
											throw new Exception("The database was not successfully closed.");
										if (blIsSuccessful == false) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Room_List.objDisplayAlertClass != null) {
				Room_List.objDisplayAlertClass.cleanUpClassVars();
				Room_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.myAppCleanup", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE:
					new AlertDialog.Builder(Room_List.this)
														...
														new DialogInterface.OnClickListener() {
															public void onClick(DialogInterface dialog,
																...
																AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
																		.getMenuInfo();
																try {
																	boolean dbOpenResult = Room_List.this.dbAppDbObj
																			.openDbAdapter();
																	if (dbOpenResult) {
																		boolean blIsSuccessful = Room_List.this.dbAppDbObj
																				.deleteRoomEntry(info.id);
																		boolean dbCloseResult = Room_List.this.dbAppDbObj
																				.closeDbAdapter();
																		if (!dbCloseResult)
																			throw new Exception(
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Room_List.displayConfirmRequest.deleteRoomEntry",
																								"");
																	errExcpError = null;
														...
														new DialogInterface.OnClickListener() {
															public void onClick(DialogInterface dialog,
																...
																try {
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Room_List.displayConfirmRequest.deleteRoomEntries",
																								"");
																	errExcpError = null;
																}// end try/catch (Exception
																	// error)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.onSaveInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Room_List.this);
			errExcpError.addToLogFile(error, "Room_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
