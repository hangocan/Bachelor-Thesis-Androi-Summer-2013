 ...
 */
public class Storage_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Storage_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj.fetchStorage();
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error, "Storage_List.fillData", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Storage_List.this,
							AddEditStorage.class);
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_STORAGE_DB_TABLE,
												MyAppDbAdapter.KEY_STORAGE
														+ ","
														+ MyAppDbAdapter.KEY_STORAGEADDRESS
														+ ","
														+ MyAppDbAdapter.KEY_STORAGE_INFO_1
														+ " DESC");
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_STORAGE_DB_TABLE);
								Storage_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Storage_List.this,
					...
					// end try
						...
						errExcpError.addToLogFile(error,
								"Storage_List.onMenuItemSelected",
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Storage_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditStorage.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_STORAGE,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_STORAGE)));
				i.putExtra(
						MyAppDbAdapter.KEY_STORAGEADDRESS,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_STORAGEADDRESS)));
				i.putExtra(
						MyAppDbAdapter.KEY_STORAGE_INFO_1,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_STORAGE_INFO_1)));
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError
					.addToLogFile(error, "Storage_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbStorage == null)
					switch (requestCode) {
					case ACTIVITY_CREATE:
						if (resultCode == RESULT_OK && intent != null) {
							String txtStorage = extras
									.getString(MyAppDbAdapter.KEY_STORAGE);
							String txtStorageaddress = extras
									.getString(MyAppDbAdapter.KEY_STORAGEADDRESS);
							String txtStorageinfo1 = extras
						...
						if (resultCode == RESULT_OK && intent != null) {
							...
							if (rowId != null) {
								...
								String strStorageinfo1 = extras
										.getString(MyAppDbAdapter.KEY_STORAGE_INFO_1);
								String strOrigStorage = extras
										.getString("OrigStorage");
								String strOrigStorageaddress = extras
										.getString("OrigStorageaddress");
								String strOrigStorageinfo1 = extras
										.getString("OrigStorageinfo1");
								// save the data to the database table
								boolean dbOpenResult = this.dbAppDbObj
										.openDbAdapter();
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.updateStorageEntry(rowId,
													strStorage,
													strStorageaddress,
													strStorageinfo1,
													strOrigStorage,
													strOrigStorageaddress,
													strOrigStorageinfo1);
									boolean dbCloseResult = this.dbAppDbObj
											.closeDbAdapter();
									if (!dbCloseResult)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error, "Storage_List.onActivityResult",
					"");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Storage_List.objDisplayAlertClass != null) {
				Storage_List.objDisplayAlertClass.cleanUpClassVars();
				Storage_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error, "Storage_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error, "Storage_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	...
	private void displayConfirmRequest(int id, final MenuItem item) {
		try {
			...
			case DELETE_CONFIRMATION_MESSAGE:
				new AlertDialog.Builder(Storage_List.this)
								...
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
										...
										AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
												.getMenuInfo();
										try {
											boolean dbOpenResult = Storage_List.this.dbAppDbObj
													.openDbAdapter();
											if (dbOpenResult) {
												boolean blIsSuccessful = Storage_List.this.dbAppDbObj
														.deleteStorageEntry(info.id);
												boolean dbCloseResult = Storage_List.this.dbAppDbObj
														.closeDbAdapter();
												if (!dbCloseResult)
													throw new Exception(
															"The database was not successfully closed.");
											...
											errExcpError
													.addToLogFile(
															error,
															"Storage_List.displayConfirmRequest.deleteStorageEntry",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
									...
									public void onClick(DialogInterface dialog,
										...
										try {
											...
											errExcpError
													.addToLogFile(
															error,
															"Storage_List.displayConfirmRequest.deleteStorageEntries",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error,
					"Storage_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error,
					"Storage_List.onSaveInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Storage_List.this);
			errExcpError.addToLogFile(error,
					"Storage_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
