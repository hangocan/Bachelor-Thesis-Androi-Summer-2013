...
public class Review_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Review_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj.fetchReview();
				startManagingCursor(mEntryCursor);
				/** Create an array to specify the fields we want to display in the
				 * list and an array of the fields we want to bind those fields to */
				String[] from = new String[] { MyAppDbAdapter.KEY_REVIEW,
						MyAppDbAdapter.KEY_REVIEW_INFO_1 };
				int[] to = new int[] { R.id.txtreview, R.id.txtreviewinfo };
				av = (ListView) findViewById(android.R.id.list);
				// Now create a simple cursor adapter and set it to display
				listRegister = new SimpleCursorAdapter(this, R.layout.reviewentryrow, mEntryCursor,
						from, to);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.fillData", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Review_List.this, AddEditReview.class);
					...
					try {
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								...
								DBTableExport objDBTableExport = new DBTableExport(Review_List.this);
								objDBTableExport.exportAsCSVFile(cursorTableQuery,
																			MyAppDbAdapter.MY_REVIEW_DB_TABLE);
								Review_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Review_List.this, new CustAlrtMsgOptnListener(
												MessageCodes.ALERT_TYPE_MSG), "Table Export",
										"Export Completed");
					...
					// end try
						...
						MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
						errExcpError.addToLogFile(	error, "Review_List.onMenuItemSelected",
															"attempting to export the "
																	+ MyAppDbAdapter.MY_REVIEW_DB_TABLE + " table.");
						errExcpError = null;
						return false;
					}// end try/catch
	...
	/** onPrepareOptionsMenu method Prepare the Screen's standard options menu to
	 * be displayed. This is called right before the menu is shown, every time it
	 * is shown. You can use this method to efficiently enable/disable items or
	 * otherwise dynamically modify the contents. The default implementation
	 * updates the system menu items based on the activity's state. Deriving
	 * classes should always call through to the base class implementation.
	 * @param Menu menu The options menu as last shown or first initialized by
	 *           onCreateOptionsMenu().
	 * @return Must return true for the menu to be displayed; if it returns false
	 *         it will not be shown. */
	@Override
	...
	/** onCreateContextMenu method Called when a context menu for the view is
	 * about to be shown. Unlike onCreateOptionsMenu(Menu), this will be called
	 * every time the context menu is about to be shown and should be populated
	 * for the view (or item inside the view for AdapterView subclasses, this can
	 * be found in the menuInfo)). Use
	 * onContextItemSelected(android.view.MenuItem) to know when an item has been
	 * selected. It is not safe to hold onto the context menu after this method
	 * returns. Called when the context menu for this view is being built. It is
	 * not safe to hold onto the menu after this method returns.
	 * @param Menu menu The context menu that is being built.
	 * @param View v The view for which the context menu is being built
	 * @param ContextMenuInfo menuInfo Extra information about the item for which
	 *           the context menu should be shown. This information will vary
	...
	/** onContextItemSelected method This hook is called whenever an item in a
	 * context menu is selected. The default implementation simply returns false
	 * to have the normal processing happen (calling the item's Runnable or
	 * sending a message to its Handler as appropriate). You can use this method
	 * for any items for which you would like to do processing without those
	 * other facilities. Use getMenuInfo() to get extra information set by the
	 * View that added this menu item. Derived classes should call through to the
	 * base class for it to perform the default menu handling.
	 * @param MenuItem item The context menu item that was selected.
	...
	/** onListItemClick method This method will be called when an item in the list
	 * is selected. Subclasses should override. Subclasses can call
	 * getListView().getItemAtPosition(position) if they need to access the data
	 * associated with the selected item.
	 * @param ListView l The ListView where the click happened.
	 * @param View v The view that was clicked within the ListView.
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				this.mEntryCursor = ((SimpleCursorAdapter) l.getAdapter()).getCursor();
				this.mEntryCursor.moveToPosition(position);
				Intent i = new Intent(this, AddEditReview.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(MyAppDbAdapter.KEY_REVIEW, this.mEntryCursor.getString(this.mEntryCursor
						.getColumnIndexOrThrow(MyAppDbAdapter.KEY_REVIEW)));
				i.putExtra(MyAppDbAdapter.KEY_REVIEW_INFO_1, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_REVIEW_INFO_1)));
				i.putExtra(MyAppDbAdapter.KEY_REVIEW_BOOKID, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_REVIEW_BOOKID)));
				i.putExtra(MyAppDbAdapter.KEY_REVIEW_AUTHORID, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_REVIEW_AUTHORID)));
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Review_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbReview == null)
					switch (requestCode) {
						case ACTIVITY_CREATE:
							if (resultCode == RESULT_OK && intent != null) {
								String txtReview = extras.getString(MyAppDbAdapter.KEY_REVIEW);
								String txtReviewinfo = extras.getString(MyAppDbAdapter.KEY_REVIEW_INFO_1);
								int spinSpinnerreviewauthor = extras
										.getInt(MyAppDbAdapter.KEY_REVIEW_AUTHORID);
								int spinSpinnerreviewbook = extras.getInt(MyAppDbAdapter.KEY_REVIEW_BOOKID);
								// save the data to the database table
								boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.createReviewEntry(	txtReview, txtReviewinfo,
								...
								// list entry
								this.intEntryListPosition = this.getListView().getSelectedItemPosition();
								break;
							} else break;
						case ACTIVITY_EDIT:
							if (resultCode == RESULT_OK && intent != null) {
								Long rowId = extras.getLong(MyAppDbAdapter.KEY_ROWID);
								if (rowId != null) {
									String strReview = extras.getString(MyAppDbAdapter.KEY_REVIEW);
									String strReviewinfo = extras
											.getString(MyAppDbAdapter.KEY_REVIEW_INFO_1);
									String strOrigReview = extras.getString("OrigReview");
									String strOrigReviewinfo = extras.getString("OrigReviewinfo");
									int spinSpinnerreviewbook = extras
											.getInt(MyAppDbAdapter.KEY_REVIEW_BOOKID);
									int OrigSpinnerreviewbook = extras.getInt("OrigSpinnerreviewbook");
									int spinSpinnerreviewauthor = extras
											.getInt(MyAppDbAdapter.KEY_REVIEW_AUTHORID);
									int OrigSpinnerreviewauthor = extras.getInt("OrigSpinnerreviewauthor");
									// save the data to the database table
									boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
									if (dbOpenResult) {
										boolean blIsSuccessful = this.dbAppDbObj
												.updateReviewEntry(	rowId, strReview, strReviewinfo,
																			spinSpinnerreviewauthor,
																			spinSpinnerreviewbook, strOrigReview,
																			strOrigReviewinfo, OrigSpinnerreviewauthor,
																			OrigSpinnerreviewbook);
										boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
										if (!dbCloseResult)
											throw new Exception("The database was not successfully closed.");
										if (blIsSuccessful == false) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Review_List.objDisplayAlertClass != null) {
				Review_List.objDisplayAlertClass.cleanUpClassVars();
				Review_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.myAppCleanup", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	
	/** getSortOption method Sets the sorting option for how the data is sorted
	 * @return void */
	/** Called after onRestoreInstanceState(Bundle), onRestart(), or onPause(),
	 * for your activity to start interacting with the user. This is a good place
	 * to begin animations, open exclusive-access devices (such as the camera),
	 * etc. the data is refreshed */
	@Override
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	 ...
	 * "QUIT" state, then the cleanup and finish code is executed. "SAVEINSTANCE"
	 * is used for when the screen orientation is changed.
	 * @return void */
	@Override
	protected void onPause() {
		...
		if (APPGlobalVars.SCR_PAUSE_CTL != null && APPGlobalVars.SCR_PAUSE_CTL.equals("QUIT")) {
	...
	/** displayConfirmRequest method Prompts user to confirm an action. Action
	 * types include: deleting an entry or clearing all entries.
	 * @param int id Row ID of the entry to delete
	 * @param MenuItem The context menu item that was selected.
	 * @return void */
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE:
					new AlertDialog.Builder(Review_List.this)
														...
														new DialogInterface.OnClickListener() {
															...
															public void onClick(DialogInterface dialog,
																...
																AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
																		.getMenuInfo();
																try {
																	boolean dbOpenResult = Review_List.this.dbAppDbObj
																			.openDbAdapter();
																	if (dbOpenResult) {
																		boolean blIsSuccessful = Review_List.this.dbAppDbObj
																				.deleteReviewEntry(info.id);
																		boolean dbCloseResult = Review_List.this.dbAppDbObj
																				.closeDbAdapter();
																		if (!dbCloseResult)
																			throw new Exception(
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Review_List.displayConfirmRequest.deleteReviewEntry",
																								"");
																	errExcpError = null;
														...
														new DialogInterface.OnClickListener() {
															...
															public void onClick(DialogInterface dialog,
																...
																try {
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Review_List.displayConfirmRequest.deleteReviewEntries",
																								"");
																	errExcpError = null;
																}// end try/catch (Exception
																	// error)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end displayConfirmRequest
	
	/** @param blContinue the blContinue to set */
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.onSaveInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			if (savedInstanceState != null) {
				this.intEntryListPosition = savedInstanceState.getInt("LISTPOSITION");
			}// end if (savedInstanceState != null)
		}// end try
		catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Review_List.this);
			errExcpError.addToLogFile(error, "Review_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	/** setSelection method Set the currently selected list item to the specified
	 * position with the adapter's data
	 * @param int position The position value that the list will change focus to.
	 * @return void */
	@Override
