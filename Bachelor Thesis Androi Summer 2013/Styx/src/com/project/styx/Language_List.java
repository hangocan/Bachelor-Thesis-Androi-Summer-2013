 ...
 */
public class Language_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Language_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj.fetchLanguage();
				 ...
				 * fields to
				 */
				String[] from = new String[] { MyAppDbAdapter.KEY_LANGUAGE
					 };
				int[] to = new int[] { R.id.txtlanguage };
				av = (ListView) findViewById(android.R.id.list);
				// Now create a simple cursor adapter and set it to display
				listRegister = new SimpleCursorAdapter(this,
						R.layout.languageentryrow, mEntryCursor, from, to);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error, "Language_List.fillData", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Language_List.this,
							AddEditLanguage.class);
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_LANGUAGE_DB_TABLE,
												MyAppDbAdapter.KEY_LANGUAGE
														+ ","
														+ MyAppDbAdapter.KEY_LANGUAGE_INFO_1
														+ " DESC");
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_LANGUAGE_DB_TABLE);
								Language_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Language_List.this,
					...
					// end try
						...
						errExcpError.addToLogFile(error,
								"Language_List.onMenuItemSelected",
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Language_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditLanguage.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_LANGUAGE,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_LANGUAGE)));
				i.putExtra(
						MyAppDbAdapter.KEY_LANGUAGE_INFO_1,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_LANGUAGE_INFO_1)));
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Language_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbLanguage == null)
					switch (requestCode) {
					case ACTIVITY_CREATE:
						if (resultCode == RESULT_OK && intent != null) {
							String txtLanguage = extras
							...
							if (dbOpenResult) {
								boolean blIsSuccessful = this.dbAppDbObj
										.createLanguageEntry(txtLanguage, txtAddinfo);
								boolean dbCloseResult = this.dbAppDbObj
										.closeDbAdapter();
								if (!dbCloseResult)
									throw new Exception(
						...
						if (resultCode == RESULT_OK && intent != null) {
							...
							if (rowId != null) {
								...
								String strAddinfo = extras
										.getString(MyAppDbAdapter.KEY_LANGUAGE_INFO_1);
								String strOrigLanguage = extras
										.getString("OrigLanguage");
								...
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.updateLanguageEntry(rowId, strLanguage,
													strAddinfo, strOrigLanguage,
													strOrigAddinfo);
									boolean dbCloseResult = this.dbAppDbObj
											.closeDbAdapter();
									if (!dbCloseResult)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error, "Language_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Language_List.objDisplayAlertClass != null) {
				Language_List.objDisplayAlertClass.cleanUpClassVars();
				Language_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error, "Language_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error, "Language_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	...
	private void displayConfirmRequest(int id, final MenuItem item) {
		try {
			...
			case DELETE_CONFIRMATION_MESSAGE:
				new AlertDialog.Builder(Language_List.this)
								...
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
										...
										AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
												.getMenuInfo();
										try {
											boolean dbOpenResult = Language_List.this.dbAppDbObj
													.openDbAdapter();
											if (dbOpenResult) {
												boolean blIsSuccessful = Language_List.this.dbAppDbObj
														.deleteLanguageEntry(info.id);
												boolean dbCloseResult = Language_List.this.dbAppDbObj
														.closeDbAdapter();
												if (!dbCloseResult)
													throw new Exception(
															"The database was not successfully closed.");
											...
											errExcpError
													.addToLogFile(
															error,
															"Language_List.displayConfirmRequest.deleteLanguageEntry",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
									...
									public void onClick(DialogInterface dialog,
										...
										try {
											...
											errExcpError
													.addToLogFile(
															error,
															"Language_List.displayConfirmRequest.deleteLanguageEntries",
															"");
											errExcpError = null;
										}// end try/catch (Exception error)
									}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error,
					"Language_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error, "Language_List.onSaveInstanceState",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Language_List.this);
			errExcpError.addToLogFile(error,
					"Language_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
