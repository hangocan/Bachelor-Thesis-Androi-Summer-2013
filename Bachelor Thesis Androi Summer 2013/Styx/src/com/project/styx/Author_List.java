 ...
 */
public class Author_List extends ListActivity {
	
	private static final int intDefaultSortOption = -1;
	static int intSortOption = Author_List.intDefaultSortOption;
	...
	private SimpleCursorAdapter authorRegister;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Author_List.onCreate", "");
			errExcpError = null;
	...
	private void fillAuthor() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				this.mAuthorCursor = this.dbAppDbObj
						.fetchAuthor(Author_List.intSortOption);
				...
				int[] to = new int[]{R.id.txtauthorfullname,
						R.id.txtauthorhomepage};

				authorlist = (ListView) findViewById(android.R.id.list);
				// Now create a simple cursor adapter and set it to display
				this.authorRegister = new SimpleCursorAdapter(this,
						R.layout.authorentryrow, mAuthorCursor, from, to);
				...
				authorRegister
						.setFilterQueryProvider(new FilterQueryProvider() {
							public Cursor runQuery(CharSequence constraint) {
								...
								return dbAppDbObj
										.fetchAuthorByName(partialItemName);
							}
						});
				authorlist.setAdapter(authorRegister);
				authorlist.setFastScrollEnabled(true);
				authorlist.setTextFilterEnabled(true);
				...
				Search.addTextChangedListener(new TextWatcher() {
					...
					public void onTextChanged(CharSequence s, int start,
						...
						if (authorRegister != null) {
							authorRegister.getFilter().filter(s.toString());
							authorlist.setAdapter(authorRegister);
						}
					}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error, "Author_List.fillAuthor", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Author_List.this,
							AddEditAuthor.class);
					...
					prefsIntent.putExtra("PREFERENCE_FILE",
							R.xml.author_view_prefs);
					prefsIntent.putExtra("PREFERENCE_KEY",
							getString(R.string.AuthorSortOpt));
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_AUTHOR_DB_TABLE,
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_AUTHOR_DB_TABLE);
								Author_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Author_List.this,
					...
					// end try
						...
						errExcpError.addToLogFile(error,
								"Author_List.onMenuItemSelected",
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Author_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mAuthorCursor != null) {
				this.mAuthorCursor = ((SimpleCursorAdapter) l.getAdapter())
						.getCursor();
				this.mAuthorCursor.moveToPosition(position);
				Intent i = new Intent(this, AddEditAuthor.class);

				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_AUTHORFULLNAME,
						this.mAuthorCursor.getString(this.mAuthorCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_AUTHORFULLNAME)));
				i.putExtra(
						MyAppDbAdapter.KEY_AUTHORHOMEPAGE,
						this.mAuthorCursor.getString(this.mAuthorCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_AUTHORHOMEPAGE)));
				i.putExtra(
						MyAppDbAdapter.KEY_AUTHOREMAIL,
						this.mAuthorCursor.getString(this.mAuthorCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_AUTHOREMAIL)));
				this.intEntryListPosition = position;
				i.putExtra("SHOW_SAVENEW", true);
				startActivityForResult(i, ACTIVITY_EDIT);
			}// end if (this.mAuthorCursor != null)
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Author_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbAuthor == null)
					switch (requestCode) {
						case ACTIVITY_CREATE :
							if (resultCode == RESULT_OK && intent != null) {
								String txtAuthorfullname = extras
										.getString(MyAppDbAdapter.KEY_AUTHORFULLNAME);
								String txtAuthorhomepage = extras
										.getString(MyAppDbAdapter.KEY_AUTHORHOMEPAGE);
								String txtAuthoremail = extras
						...
						case ACTIVITY_EDIT :
							if (resultCode == RESULT_OK && intent != null) {
								...
								if (rowId != null) {
									String strAuthor = extras
											.getString(MyAppDbAdapter.KEY_AUTHORFULLNAME);
									String strHomepage = extras
											.getString(MyAppDbAdapter.KEY_AUTHORHOMEPAGE);
									String strEmail = extras
											.getString(MyAppDbAdapter.KEY_AUTHOREMAIL);
									String strOrigAuthor = extras
											.getString("OrigAuthor");
									String strOrigHomepage = extras
											.getString("OrigHomepage");
									String strOrigEmail = extras
											.getString("OrigEmail");
									// save the data to the database table
									boolean dbOpenResult = this.dbAppDbObj
											.openDbAdapter();
									if (dbOpenResult) {
										boolean blIsSuccessful = this.dbAppDbObj
												.updateAuthorEntry(rowId,
														strAuthor, strHomepage,
														strEmail,
														strOrigAuthor,
														strOrigHomepage,
														strOrigEmail);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError
					.addToLogFile(error, "Author_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Author_List.objDisplayAlertClass != null) {
				Author_List.objDisplayAlertClass.cleanUpClassVars();
				Author_List.objDisplayAlertClass = null;
			}
			if (this.mAuthorCursor != null) {
				this.mAuthorCursor.close();
				this.mAuthorCursor = null;
			}
			if (this.authorRegister != null) {
				this.authorRegister.getCursor().close();
				this.authorRegister = null;
			}
			if (this.dbAppDbObj != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error, "Author_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected static void getSortOption(Context ctxContext) {
		try {
			GetSortOptions objGetSortOptions = new GetSortOptions(ctxContext,
					"Author_List", "AuthorSortOpt");
			Author_List.intSortOption = objGetSortOptions.getIntSortOption("");
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					ctxContext);
			errExcpError
					.addToLogFile(error, "Author_List.getSortOption",
							"An Error Occured, Sort Option set to default, sorted by rowID");
			errExcpError = null;
			Author_List.intSortOption = Author_List.intDefaultSortOption;
		}// end try/catch (Exception error)
	}// end getSortOption
	...
	protected void onResume() {
		...
		try {
			if (this.blSaveNew == true) {
				...
				// set database query class object
			this.dbAppDbObj = new MyAppDbSQL(this);
			Author_List.getSortOption(Author_List.this);
			this.fillAuthor();
			this.registerForContextMenu(this.getListView());
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error, "Author_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE :
					new AlertDialog.Builder(Author_List.this)
									...
									new DialogInterface.OnClickListener() {
										public void onClick(
											...
											AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
													.getMenuInfo();
											try {
												boolean dbOpenResult = Author_List.this.dbAppDbObj
														.openDbAdapter();
												if (dbOpenResult) {
													boolean blIsSuccessful = Author_List.this.dbAppDbObj
															.deleteAuthorEntry(info.id);
													boolean dbCloseResult = Author_List.this.dbAppDbObj
															.closeDbAdapter();
													if (!dbCloseResult)
														throw new Exception(
																"The database was not successfully closed.");
												...
												errExcpError
														.addToLogFile(
																error,
																"Author_List.displayConfirmRequest.deleteAuthorEntry",
																"");
												errExcpError = null;
											}// end try/catch (Exception error)
										}
										...
										public void onClick(
											...
											try {
												...
												errExcpError
														.addToLogFile(
																error,
																"Author_List.displayConfirmRequest.deleteAuthorEntries",
																"");
												errExcpError = null;
											}// end try/catch (Exception error)
										}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error,
					"Author_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error, "Author_List.onSaveInstanceState",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Author_List.this);
			errExcpError.addToLogFile(error,
					"Author_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
