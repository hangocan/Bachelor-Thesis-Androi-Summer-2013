...
public class Publisher_List extends ListActivity {
	/* Global Variable Declarations */
	private static final int intDefaultSortOption = -1;
	static int intSortOption = Publisher_List.intDefaultSortOption;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.publisherlist);
		fillData();
		try {
			...
			errExcpError.addToLogFile(error, "Publisher_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj.fetchPublisher(Publisher_List.intSortOption);
				...
				String[] from = new String[] { MyAppDbAdapter.KEY_PUBLISHER,
						MyAppDbAdapter.KEY_PUBLISHERHOMEPAGE };
				int[] to = new int[] { R.id.publisher, R.id.publisherhomepage };

				av = (ListView) findViewById(android.R.id.list);

				// Now create a simple cursor adapter and set it to display
				listRegister = new SimpleCursorAdapter(this,
						R.layout.publisherentryrow, mEntryCursor, from, to);
				listRegister.setFilterQueryProvider(new FilterQueryProvider() {
					public Cursor runQuery(CharSequence constraint) {
						...
						return dbAppDbObj.fetchPublisherByName(partialItemName);
					}
				});
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.fillData", "");
			errExcpError = null;
		...
		try {
			int itemId = item.getItemId();
			{
				if (itemId == R.id.new_entry_menu_option) {
					Intent intentInsrt = new Intent(Publisher_List.this,
							AddEditPublisher.class);
					...
					prefsIntent.putExtra("PREFERENCE_FILE",
							R.xml.publisher_view_prefs);
					prefsIntent.putExtra("PREFERENCE_KEY",
							getString(R.string.PublisherSortOpt));
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_PUBLISHER_DB_TABLE,
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_PUBLISHER_DB_TABLE);
								Publisher_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Publisher_List.this,
					...
					// end try
						...
						errExcpError.addToLogFile(error,
								"Publisher_List.onMenuItemSelected",
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Publisher_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditPublisher.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_PUBLISHER,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_PUBLISHER)));

				i.putExtra(	MyAppDbAdapter.KEY_PUBLISHERADDRESS,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_PUBLISHERADDRESS)));
				i.putExtra(
						MyAppDbAdapter.KEY_PUBLISHERHOMEPAGE,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_PUBLISHERHOMEPAGE)));
				i.putExtra(
						MyAppDbAdapter.KEY_PUBLISHEREMAIL,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_PUBLISHEREMAIL)));
						
				i.putExtra(
						MyAppDbAdapter.KEY_PUBLISHER_INFO_1,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_PUBLISHER_INFO_1)));
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Publisher_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbPublisher == null)

					switch (requestCode) {
					case ACTIVITY_CREATE:
						if (resultCode == RESULT_OK && intent != null) {
							String txtPublisher = extras
									.getString(MyAppDbAdapter.KEY_PUBLISHER);
							String txtPublisheraddress = extras
									.getString(MyAppDbAdapter.KEY_PUBLISHERADDRESS);
							String txtPublisherhomepage = extras
									.getString(MyAppDbAdapter.KEY_PUBLISHERHOMEPAGE);
							String txtPublisheremail = extras
									.getString(MyAppDbAdapter.KEY_PUBLISHEREMAIL);
							
							String txtPublisherinfo1 = extras
							...
							if (dbOpenResult) {
								boolean blIsSuccessful = this.dbAppDbObj
										.createPublisherEntry(txtPublisher, txtPublisheraddress,
												txtPublisherhomepage, txtPublisheremail,txtPublisherinfo1);
					...
					case ACTIVITY_EDIT:
						if (resultCode == RESULT_OK && intent != null) {
							...
							if (rowId != null) {
								...
								String strOrigPublisher = extras
										.getString("OrigPublisher");
								String strOrigPublisheraddress = extras
										.getString("OrigPublisheraddress");
								String strOrigPublisheremail = extras
										.getString("OrigPublisheremail");
								String strOrigPublisherhomepage = extras
										.getString("OrigPublisherhomepage");
										
								String strOrigPublisherinfo1 = extras
										.getString("OrigPublisherinfo1");
								...
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.updatePublisherEntry(rowId, strPublisher,
													strPublisheraddress, strPublisherhomepage,
													strPublisheremail,strPublisherinfo1,
													strOrigPublisher,
													strOrigPublisheraddress,
													strOrigPublisherhomepage, strOrigPublisheremail,
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Publisher_List.objDisplayAlertClass != null) {
				Publisher_List.objDisplayAlertClass.cleanUpClassVars();
				Publisher_List.objDisplayAlertClass = null;
			}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected static void getSortOption(Context ctxContext) {
		try {
			GetSortOptions objGetSortOptions = new GetSortOptions(ctxContext,
					"Publisher_List", "PublisherSortOpt");

			Publisher_List.intSortOption = objGetSortOptions.getIntSortOption("");
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					ctxContext);
			errExcpError
			.addToLogFile(error, "Publisher_List.getSortOption",
					"An Error Occured, Sort Option set to default, sorted by rowID");

			errExcpError = null;
			Publisher_List.intSortOption = Publisher_List.intDefaultSortOption;
		}// end try/catch (Exception error)
	}// end getSortOption
	...
	protected void onResume() {
		...
		try {
			...
			Publisher_List.getSortOption(Publisher_List.this);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	...
	private void displayConfirmRequest(int id, final MenuItem item) {
		try {
			...
			case DELETE_CONFIRMATION_MESSAGE:
				new AlertDialog.Builder(Publisher_List.this)
				...
				.setPositiveButton(R.string.alert_dialog_ok,
					...
					public void onClick(DialogInterface dialog,
						...
						AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
								.getMenuInfo();
						try {
							boolean dbOpenResult = Publisher_List.this.dbAppDbObj
									.openDbAdapter();

							if (dbOpenResult) {
								boolean blIsSuccessful = Publisher_List.this.dbAppDbObj
										.deletePublisherEntry(info.id);

								boolean dbCloseResult = Publisher_List.this.dbAppDbObj
										.closeDbAdapter();
								if (!dbCloseResult)
									throw new Exception(
											"The database was not successfully closed.");
							...
							.addToLogFile(
									error,
									"Publisher_List.displayConfirmRequest.deletePublisherEntry",
									"");
							errExcpError = null;
						}// end try/catch (Exception error)
					}
				...
				.setPositiveButton(R.string.alert_dialog_ok,
					...
					public void onClick(DialogInterface dialog,
						...
						try {
							...
							.addToLogFile(
									error,
									"Publisher_List.displayConfirmRequest.deletePublisherEntries",
									"");
							errExcpError = null;
						}// end try/catch (Exception error)
					}
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.displayConfirmRequest",
					"");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error, "Publisher_List.onSaveInstanceState",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Publisher_List.this);
			errExcpError.addToLogFile(error,
					"Publisher_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
