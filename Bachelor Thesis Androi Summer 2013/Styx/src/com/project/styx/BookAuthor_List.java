...
public class BookAuthor_List extends ListActivity {
	
	private static final int intDefaultSortOption = -1;
	private static int intSortOption = BookAuthor_List.intDefaultSortOption;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "BookAuthor_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				...
				mEntryCursor = dbAppDbObj.fetchBook(BookAuthor_List.intSortOption);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError.addToLogFile(error, "BookAuthor_List.fillData", "");
			errExcpError = null;
			...
			int itemId = item.getItemId();
			{
				if (itemId == R.id.sorting_options) {
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(MyAppDbAdapter.MY_BOOK_AUTHOR_DB_TABLE,
								...
								objDBTableExport
										.exportAsCSVFile(	cursorTableQuery,
																MyAppDbAdapter.MY_BOOK_AUTHOR_DB_TABLE);
								BookAuthor_List.objDisplayAlertClass = new MyDisplayAlertClass(
										BookAuthor_List.this,
					...
					// end try
						...
						errExcpError
								.addToLogFile(	error,
													"BookAuthor_List.onMenuItemSelected",
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditBookAuthor.class);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError
					.addToLogFile(error, "BookAuthor_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
	...
	protected void myAppCleanup() {
		try {
			if (BookAuthor_List.objDisplayAlertClass != null) {
				BookAuthor_List.objDisplayAlertClass.cleanUpClassVars();
				BookAuthor_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError.addToLogFile(	error, "BookAuthor_List.myAppCleanup",
												"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected static void getSortOption(Context ctxContext) {
		try {
			GetSortOptions objGetSortOptions = new GetSortOptions(ctxContext,
					"BookAuthor_List", "BookSortOpt");
			BookAuthor_List.intSortOption = objGetSortOptions.getIntSortOption("");
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					ctxContext);
			errExcpError
					.addToLogFile(	error, "BookAuthor_List.getSortOption",
										"An Error Occured, Sort Option set to default, sorted by rowID");
			errExcpError = null;
			BookAuthor_List.intSortOption = BookAuthor_List.intDefaultSortOption;
		}// end try/catch (Exception error)
	}// end getSortOption
	...
	protected void onResume() {
		...
		try {
			if (this.blSaveNew == true) {
				...
				Intent intentInsrt = new Intent(BookAuthor_List.this,
						AddEditBookAuthor.class);
			...
			BookAuthor_List.getSortOption(BookAuthor_List.this);
			this.fillData();
			this.registerForContextMenu(this.getListView());
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError.addToLogFile(error, "BookAuthor_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE:
					new AlertDialog.Builder(BookAuthor_List.this)
														...
														new DialogInterface.OnClickListener() {
															...
															public void onClick(	DialogInterface dialog,
																...
																AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
																		.getMenuInfo();
																try {
																	boolean dbOpenResult = BookAuthor_List.this.dbAppDbObj
																			.openDbAdapter();
																	if (dbOpenResult) {
																		boolean blIsSuccessful = BookAuthor_List.this.dbAppDbObj
																				.deleteBookEntry(info.id);
																		boolean dbCloseResult = BookAuthor_List.this.dbAppDbObj
																				.closeDbAdapter();
																		if (!dbCloseResult) throw new Exception(
																				"The database was not successfully closed.");
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"BookAuthor_List.displayConfirmRequest.deleteBookEntry",
																								"");
																	errExcpError = null;
														...
														new DialogInterface.OnClickListener() {
															...
															public void onClick(	DialogInterface dialog,
																...
																try {
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"BookAuthor_List.displayConfirmRequest.deleteBookEntries",
																								"");
																	errExcpError = null;
																}// end try/catch (Exception
																	// error)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError
					.addToLogFile(error, "BookAuthor_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError.addToLogFile(	error,
												"BookAuthor_List.onSaveInstanceState",
												"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					BookAuthor_List.this);
			errExcpError.addToLogFile(	error,
												"BookAuthor_List.onRestoreInstanceState",
												"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
