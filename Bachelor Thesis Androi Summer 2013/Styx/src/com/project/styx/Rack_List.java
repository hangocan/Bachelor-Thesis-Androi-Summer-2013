 ...
 */
/** Class for displaying a list of data, activity screen and options. */
public class Rack_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.racklist);
		fillData();
		try {
			...
			errExcpError.addToLogFile(error, "Rack_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				this.mEntryCursor = this.dbAppDbObj.fetchRack();
				this.startManagingCursor(this.mEntryCursor);
				String[] from = new String[] { MyAppDbAdapter.KEY_RACK_LABEL };
				int[] to = new int[] { R.id.racklabel };
				listRegister = new SimpleCursorAdapter(Rack_List.this, R.layout.rackentryrow,
						this.mEntryCursor, from, to) {};
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.fillData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end fillData
	 ...
	 * @param TextView v The textview being processed
	 * @param String text used for passing the processed result
	 * @return void */
		...
		try {
			switch (item.getItemId()) {
				case R.id.new_entry_menu_option:
					Intent intentInsrt = new Intent(Rack_List.this, AddEditRack.class);
				...
				case R.id.export_list:
					...
					try {
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(MyAppDbAdapter.MY_RACK_DB_TABLE,
																MyAppDbAdapter.KEY_ROWID + " ASC");
								this.startManagingCursor(this.mEntryCursor);
								boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
								if (!dbCloseResult)
									throw new Exception("The database was not successfully closed.");
								DBTableExport objDBTableExport = new DBTableExport(Rack_List.this);
								objDBTableExport.exportAsCSVFile(cursorTableQuery,
																			MyAppDbAdapter.MY_RACK_DB_TABLE);
								Rack_List.objDisplayAlertClass = new MyDisplayAlertClass(Rack_List.this,
						...
						MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
						errExcpError.addToLogFile(	error, "Rack_List.onMenuItemSelected",
															"attempting to export the "
																	+ MyAppDbAdapter.MY_RACK_DB_TABLE + " table.");
						errExcpError = null;
						return false;
					}// end try/catch
				case R.id.quit:
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditRack.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(MyAppDbAdapter.KEY_RACK_LABEL, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_RACK_LABEL)));
				i.putExtra(MyAppDbAdapter.KEY_RACK_INFO_1, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_RACK_INFO_1)));
				i.putExtra(MyAppDbAdapter.KEY_RACK_ROOMID, this.mEntryCursor
						.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_RACK_ROOMID)));
				// bgh 08/26/2010 1.03
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Rack_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbRack == null)
					switch (requestCode) {
						case ACTIVITY_CREATE:
							if (resultCode == RESULT_OK && intent != null) {
								String txtRacklabel = extras.getString(MyAppDbAdapter.KEY_RACK_LABEL);
								String txtRackinfo1 = extras.getString(MyAppDbAdapter.KEY_RACK_INFO_1);
								int spinSpinnerrack = extras.getInt(MyAppDbAdapter.KEY_RACK_ROOMID);
								// save the data to the database table
								boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.createRackEntry(txtRacklabel, txtRackinfo1, spinSpinnerrack);
									boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
									if (!dbCloseResult)
										throw new Exception("The database was not successfully closed.");
									if (blIsSuccessful == false) {
										Rack_List.objDisplayAlertClass = new MyDisplayAlertClass(
												Rack_List.this, new CustAlrtMsgOptnListener(
						...
						case ACTIVITY_EDIT:
							if (resultCode == RESULT_OK && intent != null) {
								...
								if (rowId != null) {
									String strRacklabel = extras.getString(MyAppDbAdapter.KEY_RACK_LABEL);
									String strRackinfo1 = extras.getString(MyAppDbAdapter.KEY_RACK_INFO_1);
									String strOrigRacklabel = extras.getString("OrigRacklabel");
									String strOrigRackinfo1 = extras.getString("OrigRackinfo1");
									int spinSpinnerrack = extras.getInt(MyAppDbAdapter.KEY_RACK_ROOMID);
									int OrigSpinnerrack = extras.getInt("OrigSpinnerrack");
									// save the data to the database table
									boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
									if (dbOpenResult) {
										boolean blIsSuccessful = this.dbAppDbObj
												.updateRackEntry(rowId, strRacklabel, strRackinfo1,
																		spinSpinnerrack, strOrigRacklabel,
																		strOrigRackinfo1, OrigSpinnerrack);
										boolean dbCloseResult = this.dbAppDbObj.closeDbAdapter();
										if (!dbCloseResult)
											throw new Exception("The database was not successfully closed.");
										if (blIsSuccessful == false) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Rack_List.objDisplayAlertClass != null) {
				Rack_List.objDisplayAlertClass.cleanUpClassVars();
				Rack_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.myAppCleanup", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	...
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE:
					new AlertDialog.Builder(Rack_List.this)
														...
														new DialogInterface.OnClickListener() {
															public void onClick(DialogInterface dialog,
																...
																AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
																		.getMenuInfo();
																try {
																	boolean dbOpenResult = Rack_List.this.dbAppDbObj
																			.openDbAdapter();
																	if (dbOpenResult) {
																		boolean blIsSuccessful = Rack_List.this.dbAppDbObj
																				.deleteRackEntry(info.id);
																		boolean dbCloseResult = Rack_List.this.dbAppDbObj
																				.closeDbAdapter();
																		if (!dbCloseResult)
																			throw new Exception(
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Rack_List.displayConfirmRequest.deleteRackEntry",
																								"");
																	errExcpError = null;
														...
														new DialogInterface.OnClickListener() {
															public void onClick(DialogInterface dialog,
																...
																try {
																	...
																	errExcpError
																			.addToLogFile(	error,
																								"Rack_List.displayConfirmRequest.deleteRackEntries",
																								"");
																	errExcpError = null;
																}// end try/catch (Exception
																	// error)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.onSaveInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(Rack_List.this);
			errExcpError.addToLogFile(error, "Rack_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
