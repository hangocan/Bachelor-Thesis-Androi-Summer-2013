 ...
 */
/** Class for displaying a list of data, activity screen and options. */
public class Shelf_List extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			errExcpError.addToLogFile(error, "Shelf_List.onCreate", "");
			errExcpError = null;
	...
	private void fillData() {
		...
		try {
			...
			if (dbOpenResult) {
				this.mEntryCursor = this.dbAppDbObj.fetchShelf();
				this.startManagingCursor(this.mEntryCursor);
				String[] from = new String[]{MyAppDbAdapter.KEY_SHELF_LABEL};
				int[] to = new int[]{R.id.shelflabel};
				listRegister = new SimpleCursorAdapter(Shelf_List.this,
						R.layout.shelfentryrow, this.mEntryCursor, from, to) {
				};
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error, "Shelf_List.fillData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end fillData
		...
		try {
			switch (item.getItemId()) {
				case R.id.new_entry_menu_option :
					Intent intentInsrt = new Intent(Shelf_List.this,
							AddEditShelf.class);
						...
						if (this.dbAppDbObj != null) {
							...
							if (dbOpenResult) {
								cursorTableQuery = this.dbAppDbObj
										.exportTableQuery(
												MyAppDbAdapter.MY_SHELF_DB_TABLE,
								...
								objDBTableExport.exportAsCSVFile(
										cursorTableQuery,
										MyAppDbAdapter.MY_SHELF_DB_TABLE);
								Shelf_List.objDisplayAlertClass = new MyDisplayAlertClass(
										Shelf_List.this,
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Shelf_List.onMenuItemSelected",
	 ...
	 * be displayed. This is called right before the menu is shown, every time
	 * it is shown. You can use this method to efficiently enable/disable items
	 * or otherwise dynamically modify the contents. The default implementation
	 ...
	 * for the view (or item inside the view for AdapterView subclasses, this
	 * can be found in the menuInfo)). Use
	 * onContextItemSelected(android.view.MenuItem) to know when an item has
	 * been selected. It is not safe to hold onto the context menu after this
	 * method returns. Called when the context menu for this view is being
	 * built. It is not safe to hold onto the menu after this method returns.
	 * 
	 * @param Menu
	 ...
	 * View that added this menu item. Derived classes should call through to
	 * the base class for it to perform the default menu handling.
	 * 
	 * @param MenuItem
	...
	/**
	 * onListItemClick method This method will be called when an item in the
	 * list is selected. Subclasses should override. Subclasses can call
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			if (this.mEntryCursor != null) {
				...
				Intent i = new Intent(this, AddEditShelf.class);
				i.putExtra(MyAppDbAdapter.KEY_ROWID, id);
				i.putExtra(
						MyAppDbAdapter.KEY_SHELF_LABEL,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_SHELF_LABEL)));
				i.putExtra(
						MyAppDbAdapter.KEY_SHELF_INFO_1,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_SHELF_INFO_1)));
				i.putExtra(
						MyAppDbAdapter.KEY_SHELF_RACKID,
						this.mEntryCursor.getString(this.mEntryCursor
								.getColumnIndexOrThrow(MyAppDbAdapter.KEY_SHELF_RACKID)));
				// bgh 08/26/2010 1.03
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
			errExcpError.addToLogFile(error, "Shelf_List.onListItemClick", "");
			errExcpError = null;
		}// end try/catch
	}// end onListItemClick
		...
		try {
			if (intent != null) {
				...
				if (extras != null) {
					...
					}// end if (this.dbShelf == null)
					switch (requestCode) {
						case ACTIVITY_CREATE :
							if (resultCode == RESULT_OK && intent != null) {
								String txtShelflabel = extras
										.getString(MyAppDbAdapter.KEY_SHELF_LABEL);
								String txtShelfinfo1 = extras
										.getString(MyAppDbAdapter.KEY_SHELF_INFO_1);
								int spinSpinnershelf = extras
										.getInt(MyAppDbAdapter.KEY_SHELF_RACKID);
								// save the data to the database table
								boolean dbOpenResult = this.dbAppDbObj
										.openDbAdapter();
								if (dbOpenResult) {
									boolean blIsSuccessful = this.dbAppDbObj
											.createShelfEntry(txtShelflabel,
													txtShelfinfo1,
													spinSpinnershelf);
									boolean dbCloseResult = this.dbAppDbObj
											.closeDbAdapter();
						...
						case ACTIVITY_EDIT :
							if (resultCode == RESULT_OK && intent != null) {
								...
								if (rowId != null) {
									String strShelflabel = extras
											.getString(MyAppDbAdapter.KEY_SHELF_LABEL);
									String strShelfinfo1 = extras
											.getString(MyAppDbAdapter.KEY_SHELF_INFO_1);
									String strOrigShelflabel = extras
											.getString("OrigShelflabel");
									String strOrigShelfinfo1 = extras
											.getString("OrigShelfinfo1");
									int spinSpinnershelf = extras
											.getInt(MyAppDbAdapter.KEY_SHELF_RACKID);
									int OrigSpinnershelf = extras
											.getInt("OrigSpinnershelf");
									// save the data to the database table
									boolean dbOpenResult = this.dbAppDbObj
											.openDbAdapter();
									if (dbOpenResult) {
										boolean blIsSuccessful = this.dbAppDbObj
												.updateShelfEntry(rowId,
														strShelflabel,
														strShelfinfo1,
														spinSpinnershelf,
														strOrigShelflabel,
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error, "Shelf_List.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	protected void myAppCleanup() {
		try {
			...
			if (Shelf_List.objDisplayAlertClass != null) {
				Shelf_List.objDisplayAlertClass.cleanUpClassVars();
				Shelf_List.objDisplayAlertClass = null;
			}
			if (this.mEntryCursor != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error, "Shelf_List.myAppCleanup",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end myAppCleanup
	 ...
	 * camera), etc. the data is refreshed
	 */
	@Override
	protected void onResume() {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error, "Shelf_List.resumeData", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}// end onResume
	 ...
	 * "QUIT" state, then the cleanup and finish code is executed.
		...
		try {
			...
			switch (id) {
				case DELETE_CONFIRMATION_MESSAGE :
					new AlertDialog.Builder(Shelf_List.this)
									...
									new DialogInterface.OnClickListener() {
										public void onClick(
											...
											AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
													.getMenuInfo();
											try {
												boolean dbOpenResult = Shelf_List.this.dbAppDbObj
														.openDbAdapter();
												if (dbOpenResult) {
													boolean blIsSuccessful = Shelf_List.this.dbAppDbObj
															.deleteShelfEntry(info.id);
													boolean dbCloseResult = Shelf_List.this.dbAppDbObj
															.closeDbAdapter();
													if (!dbCloseResult)
														throw new Exception(
																"The database was not successfully closed.");
												...
												errExcpError
														.addToLogFile(
																error,
																"Shelf_List.displayConfirmRequest.deleteShelfEntry",
																"");
												errExcpError = null;
										...
										public void onClick(
											...
											try {
												...
												errExcpError
														.addToLogFile(
																error,
																"Shelf_List.displayConfirmRequest.deleteShelfEntries",
																"");
												errExcpError = null;
											}// end try/catch (Exception
												// error)
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error,
					"Shelf_List.displayConfirmRequest", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error, "Shelf_List.onSaveInstanceState",
					"no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					Shelf_List.this);
			errExcpError.addToLogFile(error,
					"Shelf_List.onRestoreInstanceState", "no prompt");
			errExcpError = null;
		}// end try/catch (Exception error)
