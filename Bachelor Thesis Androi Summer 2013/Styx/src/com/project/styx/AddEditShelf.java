...
public class AddEditShelf extends Activity {
	
	/** Define variables to use in the activity **/
	private EditText txtShelflabel;
	private EditText txtShelfinfo1;
	private String strOrigShelf = "";
	private String strOrigShelfinfo1 = "";
	public Integer OrigSpinnershelf = 0;
	private Long mRowId;
	private Spinner spinnershelf;
	private MyAppDbSQL dbAppDbObj;
	private Cursor spinnershelfcursor;
	...
	private OnFocusChangeListener txtShelfOnFocusChangeListener = new OnFocusChangeListener() {
	...
	private OnFocusChangeListener txtShelfinfo1OnFocusChangeListener = new OnFocusChangeListener() {
	...
	protected void myAppCleanup() {
		...
		if (this.spinnershelfcursor != null) {
			this.spinnershelfcursor.close();
			this.spinnershelfcursor = null;
		}
	...
	private void myCancelMethod() {
		...
		bundle.putBoolean("IS_SAVE_NEW", AddEditShelf.this.blSaveNew);
	...
	private void mySaveDataMethod() {
		...
		if (this.txtShelflabel == null) {
			this.txtShelflabel = (EditText) findViewById(R.id.txtShelflabel);
		}
		try {
			/** Define the condition command for must filled field ***************************************/
			strShelf = this.txtShelflabel.getText().toString();
			if (strShelf.contentEquals("")) {
				objDisplayAlertClass = new MyDisplayAlertClass(AddEditShelf.this,
						new CustAlrtMsgOptnListener(MessageCodes.ALERT_TYPE_MSG),
						"Missing Required Shelflabel ", "Please enter  the shelf ");
				boolSaveOk = false;
				txtShelflabel.requestFocus();
			}
			/******************************************************************************************/
			strShelfinfo1 = this.txtShelfinfo1.getText().toString();
			// ////////////////////////////////////////////////////////////
			spinSpinnershelf = (int) this.spinnershelf.getSelectedItemId();
			// ////////////////////////////////////////////////////////////
			/** Save file command **/
			if (boolSaveOk == true) {
				APPGlobalVars.SCR_PAUSE_CTL = "SAVE";
				Bundle bundle = new Bundle();
				bundle.putString(MyAppDbAdapter.KEY_SHELF_LABEL, this.txtShelflabel
						.getText().toString());
				bundle.putString(MyAppDbAdapter.KEY_SHELF_INFO_1, strShelfinfo1);
				bundle.putInt(MyAppDbAdapter.KEY_SHELF_RACKID, spinSpinnershelf);
				bundle.putBoolean("SHOW_FILTER", true);
				if (mRowId != null) {
					bundle.putLong(MyAppDbAdapter.KEY_ROWID, mRowId);
				}
				if (((!strOrigShelf.equals(strShelf)) && (!strOrigShelf.equals("")))) {
					bundle.putString("OrigShelf", strOrigShelf);
				} else {
					bundle.putString("OrigShelf", "");
				}// end if
				if (((!strOrigShelfinfo1.equals(strShelfinfo1)) && (!strOrigShelfinfo1
						.equals("")))) {
					bundle.putString("OrigShelfinfo1", strOrigShelfinfo1);
				} else {
					bundle.putString("OrigShelfinfo1", "");
				}// end if
					// ///////////////////////////////////////////////////////
				if (((!OrigSpinnershelf.equals(spinSpinnershelf)) && (!OrigSpinnershelf
						.equals(0)))) {
					bundle.putInt("OrigSpinnershelf", OrigSpinnershelf);
				} else {
					bundle.putInt("OrigSpinnershelf", 0);
				}// end if
					// ////////////////////////////////////////////////////////////
				bundle.putBoolean("IS_SAVE_NEW", AddEditShelf.this.blSaveNew);
				Intent mIntent = new Intent();
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(error, "AddEditShelf.mySaveDataMethod", "");
			errExcpError = null;
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		try {
			AddEditShelf.this.blSaveNew = false;
			txtShelflabel = (EditText) findViewById(R.id.txtShelflabel);
			txtShelfinfo1 = (EditText) findViewById(R.id.txtShelfinfo1);
			// ///////////////////////////////////////////////
			this.spinnershelf = (Spinner) findViewById(R.id.spinnershelf);
			// ///////////////////////////////////////////////
			if (intent != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(error, "AddEditShelf.onActivityResult", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.add_edit_shelf);
		fillspinnershelf();
		fillBooklist();
		try {
			try {
				...
				if (extras != null) {
					 ...
					 * will be keep there **/
					if (strShelf != null && !strShelf.equals("")
							&& !strShelf.equals(" ")) {
						txtShelflabel.setText(strShelf);
						strOrigShelf = strShelf;
					}
					if (strShelfinfo1 != null && !strShelfinfo1.equals("")
							&& !strShelfinfo1.equals(" ")) {
						txtShelfinfo1.setText(strShelfinfo1);
						strOrigShelfinfo1 = strShelfinfo1;
					}
					Integer value = Integer.valueOf(intSpinnershelf);
					if (value != null && intSpinnershelf != 0) {
						OrigSpinnershelf = intSpinnershelf;
					}
				}// end if
				/*** This code set event text focus change listeners */
				txtShelflabel
						.setOnFocusChangeListener(txtShelfOnFocusChangeListener);
				txtShelfinfo1
						.setOnFocusChangeListener(txtShelfinfo1OnFocusChangeListener);
			}// end try statements
			catch (Exception error) {
				MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
				errExcpError.addToLogFile(error, "AddEditShelf.onCreate", "");
				errExcpError = null;
			}// end try/catch (Exception error)
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(this);
	...
	private void fillspinnershelf() {
		try {
			boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
			if (dbOpenResult) {
				spinnershelfcursor = dbAppDbObj.fetchRack();
				this.startManagingCursor(this.spinnershelfcursor);
				spinnershelfcursor.moveToFirst();
				...
				spinneradapter = new SimpleCursorAdapter(AddEditShelf.this,
						android.R.layout.simple_spinner_item,
						this.spinnershelfcursor, from, to);
				spinnershelf.setAdapter(this.spinneradapter);
				Bundle extras = getIntent().getExtras();
				int intSpinnershelf = Integer.parseInt(extras
						.getString(MyAppDbAdapter.KEY_SHELF_RACKID));
				for (int i = 0; i < spinnershelf.getAdapter().getCount(); i++) {
					int id = (int) spinnershelf.getItemIdAtPosition(i);
					if (id == intSpinnershelf) {
						spinnershelf.setSelection(i);
					}
				}
			}
		} catch (Exception error) {
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(error, "AddEditShelf.fillspinnershelf ", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	}
	private void fillBooklist() {
		try {
			boolean dbOpenResult = this.dbAppDbObj.openDbAdapter();
			if (dbOpenResult) {
				/** Open the database and let mshelfCursor get the data */
				this.mBookCursor = this.dbAppDbObj.fetchBookonshelf(bRowId);
				this.startManagingCursor(this.mBookCursor);
				String[] from = new String[] { MyAppDbAdapter.KEY_TITLE };
				int[] to = new int[] { R.id.Booktitle };
				listbook = (ListView) findViewById(R.id.listbookonshelf);
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(error, "AddEditShelf.fillbook", "");
			errExcpError = null;
		...
		try {
			switch (keyCode) {
				case KeyEvent.KEYCODE_ENTER:
					if (txtShelflabel.hasFocus()) {
						txtShelfinfo1.requestFocus();
						return true;
					}
				default:
					return false;
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(error, "AddEditShelf.onKeyUp", "");
			errExcpError = null;
			return false;
		}// end try/catch (Exception error)
	}// end onKeyUp
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		if (itemId == R.id.save_menu) {
			...
			Intent intentQuitActivity = new Intent(AddEditShelf.this,
					Shelf_List.class);
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		try {
			...
			if (savedInstanceState != null) {
				String strShelf = savedInstanceState
						.getString(MyAppDbAdapter.KEY_SHELF_LABEL);
				String strShelfinfo1 = savedInstanceState
						.getString(MyAppDbAdapter.KEY_SHELF_INFO_1);
				int intSpinnershelf = savedInstanceState
			...
			errExcpError.addToLogFile(	error,
												"AddEditShelf.onRestoreInstanceState", "");
			errExcpError = null;
		}// end try/catch (Exception error)
	...
	public void onSaveInstanceState(Bundle savedInstanceState) {
		try {
			...
			savedInstanceState.putString(	MyAppDbAdapter.KEY_SHELF_LABEL,
													txtShelflabel.getText().toString());
			savedInstanceState.putString(	MyAppDbAdapter.KEY_SHELF_INFO_1,
													txtShelfinfo1.getText().toString());
			savedInstanceState.putInt(	MyAppDbAdapter.KEY_SHELF_RACKID,
												(int) spinnershelf.getSelectedItemId());
			if (mRowId != null) {
			...
			MyErrorLog<Exception> errExcpError = new MyErrorLog<Exception>(
					AddEditShelf.this);
			errExcpError.addToLogFile(	error, "AddEditShelf.onSaveInstanceState",
												"");
			errExcpError = null;
		}// end try/catch (Exception error)
